...
def discretise_by_point_monthly(df_raincells, simulation_length_years,InputTimeSeries, timestep_length=1.0):
    ...
    """
    Start = pd.to_datetime(str(int(InputTimeSeries.index.year[0]))+str('-01-01 00:00:00'),format='%Y-%m-%d %H:%M:%S')
    End = pd.to_datetime(str(int(InputTimeSeries.index.year[0])+simulation_length_years-1)+str('-12-31 23:00:00'),format='%Y-%m-%d %H:%M:%S')
...
def Get_CEH_GEAR_RF(DIRECTORY, GRID_INDEX):
    os.chdir(DIRECTORY)
    ZARR = glob.glob("*.zarr")
    basin_grid = pd.read_csv("/home/users/azhar199/DATA/CEH_GEAR/CEH_GEAR_GRIDS.csv")
    y_idx = int(basin_grid.loc[GRID_INDEX, "NorthingIndex"])
    x_idx = int(basin_grid.loc[GRID_INDEX, "EastingIndex"])
    ALLF = []
    for z in ZARR:
        ds = xr.open_zarr(z, consolidated=False)
        series = ds["rainfall_amount"].isel(x=x_idx,y=y_idx)
        rain = series.compute().values
        raindf = pd.DataFrame({"datetime": ds["time"].values,"value": rain,})
        ALLF.append(raindf)
    FINDF = pd.concat(ALLF, axis=0).reset_index(drop=True)
    FINDF["datetime"] = pd.to_datetime(FINDF["datetime"])
    FINDF = FINDF.sort_values("datetime").reset_index(drop=True)
    return FINDF


FINDF = Get_CEH_GEAR_RF('/home/users/azhar199/DATA/CEH_GEAR/CEH_ZARR',168154)
FINDF['datetime'] = pd.to_datetime(FINDF['datetime'],format='%Y-%m-%d %H:%M:%S') 
FINDF.index = FINDF['datetime']
SD = {12: 1, 1: 1, 2: 1, 3: 2, 4: 2, 5: 2, 6: 3, 7: 3, 8: 3, 9: 4, 10: 4, 11: 4}    

ALLDF = prepare_point_timeseries(FINDF, season_definitions=SD, completeness_threshold=0, 
                                 durations=['1H','24H','72H','1M'], outlier_method='trim',
...
def GetMonthLengths2(NUM):
    n_years = NUM
    Start = pd.to_datetime(str(int(FINDF.index.year[0]))+str('-01-01 00:00:00'),format='%Y-%m-%d %H:%M:%S')
    End = pd.to_datetime(str(int(FINDF.index.year[0]+n_years-1))+str('-12-31 23:00:00'),format='%Y-%m-%d %H:%M:%S')
    DatesVect = pd.date_range(start=Start,end=End,freq="H")
    monthly_df = pd.DataFrame({'Year':DatesVect.year,'Month':DatesVect.month})
    HRCOUNT = monthly_df.groupby(['Year', 'Month']).size().reset_index(name='n_hours')
...
for i in range(n_realizations):
    ...
    HR = discretise_by_point_monthly(sim_df, simulation_length_years=n_years,InputTimeSeries = FINDF)
...
obs_monthly_stats = pd.DataFrame({'Month': range(1, 13), 'Mean': obs_mon_mean.values, 
                                  'Variance': obs_mon_var.values, 'Skewness': obs_mon_skew.values,
...
ax6.set_title("1-Hour Rainfall Depths", weight='bold')
