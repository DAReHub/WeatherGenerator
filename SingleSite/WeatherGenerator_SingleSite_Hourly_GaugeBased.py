...
## Stages in Running the rainfall generator
...
def GetMonthStats(ListofDFs,WET_THRESHOLD):
    '''
    This function requires outputs from the functions PrepareTimeSeriesPoint which is ListofDFs
    '''
    dc = {
...
### This marks the end of functions relevant to rainfall generator stage 
...
## This marks the end of functions for both the rainfall and non-rainfall weather generator ##
## From here we see implementation of the rainfall generator functions and the outputs from it are 
...
def GetMonthLengths2(NUM):
    n_years = NUM
    Start = pd.to_datetime(str(int(dfb.index.year[0]))+str('-01-01 00:00:00'),format='%Y-%m-%d %H:%M:%S')
    End = pd.to_datetime(str(int(dfb.index.year[0]+n_years-1))+str('-12-31 23:00:00'),format='%Y-%m-%d %H:%M:%S')
    DatesVect = pd.date_range(start=Start,end=End,freq="H")
    monthly_df = pd.DataFrame({'Year':DatesVect.year,'Month':DatesVect.month})
    HRCOUNT = monthly_df.groupby(['Year', 'Month']).size().reset_index(name='n_hours')
...
def getMonthlySimulations(YearNumber, MonthNumber, LATITUDE_DEGREES, LONGITUDE_DEGREES, point_elevation):
    simulations = [simulate_daily_weather_point(RAINFALL_REALIZATIONS = all_realizations_df, year = YearNumber, month = MonthNumber, n_realizations = n_realizations, LATITUDE_DEGREES = LATITUDE_DEGREES,
                                                ...
                                                parameters = REGRESSED_SERIES[1], timestep = 1, output_types = ['point'], n_points = {'point': 1}, transformations = transformations_fixed, transformed_statistics_dict = transformed_statistics_dict, 
...
# YEARSIMS = []
# for b in np.unique(all_realizations_df['Year']):
#     print(b)
#     yearsims = [getMonthlySimulations(YearNumber = b, MonthNumber = j, LATITUDE_DEGREES = 51.758, LONGITUDE_DEGREES = -1.578, point_elevation = 78) for j in list(range(1,13))]
#     YEARSIMS.append(yearsims)
...
## Plot and verify the outputs from the non-rainfall generator
