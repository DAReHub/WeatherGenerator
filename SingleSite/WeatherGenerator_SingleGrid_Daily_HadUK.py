...
def prepare_haduk_weatherseries(prcp_file_path,tmax_file_path,tmin_file_path,NUM):
    ...
    for i in np.arange(0,len(RF_FILES),1):
        ...
        RF = READ_RF.variables['rainfall'][:,GRIDS['LatIndex'][NUM],GRIDS['LonIndex'][NUM]]
    
        READ_TMAX = Dataset(TEMP_MAX_FILES[i])
        tmax = READ_TMAX.variables['tasmax'][:,GRIDS['LatIndex'][NUM],GRIDS['LonIndex'][NUM]]
    
        READ_TMIN = Dataset(TEMP_MIN_FILES[i])
        tmin = READ_TMIN.variables['tasmin'][:,GRIDS['LatIndex'][NUM],GRIDS['LonIndex'][NUM]]
...
INPUT_WEATHER_SERIES = prepare_weather_series(input_timeseries = daily_ws, input_variables = input_variables,
                                              calculation_period = [CP1,CP2], completeness_threshold = 0,
...
def getMonthlySimulations(YearNumber, MonthNumber, LATITUDE_DEGREES, LONGITUDE_DEGREES, point_elevation):
    simulations = [simulate_daily_weather_point(RAINFALL_REALIZATIONS = all_realizations_df, year = YearNumber, month = MonthNumber, n_realizations = n_realizations, LATITUDE_DEGREES = LATITUDE_DEGREES,
                                                ...
                                                output_variables = ['temp_avg', 'dtr'], wet_threshold = 0.2, season_length = 'month', wind_height = 2, offset_df = OFFSET_DF, base_seed = base_seed + I, 
